%Dynamic behavior of the DC generator during a temporary change in the (current) load
clear all
clc 
...
We=2*(pi/60)*(p/2)*n;
%Calculating the mutual inductance between the shunt field and armature winding(H)
LAF=Vf/(If*We);
...
LFFs=taufs*rfs;
%L: Vector for all self and mutual inductances
L=[LAF LAFs LFF LFFs LAA LFs];
%R:Vector for all resistances
R=[rf rfs ra];
%Value of applied voltage (V) and initial calculated curent 
Ifs1=75;
wr=29.8395;
wr1=wr;
%Dynamic behavior of the DC Generator for 50% increase in load
%with constant lspeed
t01=0;
tfinal1=5;
tspan1=[t01,tfinal1];
I01=[64.05975 -10.94025 -1432.7325];
...
Te1=2*(-LAF.*out1(:,1).*out1(:,2)+LAFs*Ifs1*out1(:,1));
 ...
 wr=29.8395;
 wr2=wr;
%Dynamic behavior of the DC generator for a restablishment in load to initial value 
t02=tfinal1;
tfinal2=10;
tspan2=[t02,tfinal2];
